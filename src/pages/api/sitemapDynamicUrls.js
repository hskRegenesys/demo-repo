...
export default async (_req, res) => {
  try {
    ...
    for (const entry of sitemapData) {
      const slugExists = xmlObject.urlset.url.some((url) =>
        url.loc[0].endsWith(`/blog/${entry.slug}`)
      );
      if (!slugExists) {
    ...
    res
      .status(200)
      .json({ success: true, message: "XML file updated successfully" });
  } catch (error) {
    console.error("Error:", error);
    res
      .status(500)
      .json({ success: false, message: "Internal sitemap server error" });
  }
};
