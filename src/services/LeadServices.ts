...
class LeadService {
    ...
    async saveLead(params: any) {
        ...
        try {
            ...
            salesforceParam.Interested_Topic = this.salesforceProgramOfIngterest.get(params.interested_topic);
            salesforceParam.recordTypeId = this.drLeadRecordTypeId;
            const salesforceResponse = await this.salesforceServer.post(apiEndPoints.salesforceApi, salesforceParam);
